#!/usr/bin/env bash
# Runs unit, integration, and end-to-end tests inside the built container.
# This file is called from the host.
...
exec docker run --rm --network=none --entrypoint /app/tests/run_tests "$IMAGE" "$@"

...
config.output_dir = args.output_dir  #@param
# config.source_file_patterns = [args.source_file_patterns]  #@param
...
# the amount of audio to load at a time
...
# I think we should be able to ignore this too. 
# we want 
config.tf_record_shards = 1  #@param
...
# we now need to call embed on the embedding model, passing 60s chunks as np arrays
# 1. use soundfile to load a segment of audio
max_segments = 15
segment_length = 60
for segment_num in range(0..max_segments):
  offset_s = segment_length * segment_num

  audio = audio_utils.load_audio_window(
      args.source_file, offset_s, 
      config.embed_fn_config.model_config.sample_rate, segment_length
  )

  if len(audio) > 1:
    segment_embedding = embedding_model.embed(audio)
